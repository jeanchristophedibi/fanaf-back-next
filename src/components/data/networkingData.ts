...
export async function fetchNetworkingRequests(
  ...
  try {
    ...
    for (const requestTarget of targetsToLoad) {
      ...
      while (hasMore) {
        try {
          ...
          if (is404) {
            console.warn(`[fetchNetworkingRequests] ⚠️ ${typeLabel} - Page ${page}: Type non trouvé (404), arrêt du chargement pour ce type`);
          } else {
            console.warn(`[fetchNetworkingRequests] ⚠️ ${typeLabel} - Page ${page}:`, errorDetails);
          }
    ...
    // Logger l'erreur mais retourner les rendez-vous déjà chargés (ou un tableau vide)
    // plutôt que de lancer une exception qui bloquerait React Query
    console.error('[fetchNetworkingRequests] ❌ Erreur globale lors du chargement des rendez-vous:', {
      error: error?.message || String(error),
      errorType: typeof error,
      errorName: error?.name,
      rendezVousLoaded: allRequests.length,
    });
    // Retourner les rendez-vous déjà chargés (s'il y en a) plutôt que de bloquer
    return allRequests;
  }
}
