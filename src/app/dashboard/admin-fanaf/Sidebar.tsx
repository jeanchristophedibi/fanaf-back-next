...
import { Home, Users, Building2, Calendar, ChevronDown, ScanLine, Coins, RefreshCw } from 'lucide-react';
...
interface AdminFanafSidebarProps {
  activeNav: NavItem;
  onNavChange: (nav: NavItem) => void;
  onSwitchProfile?: () => void;
}

export function AdminFanafSidebar({ activeNav, onNavChange, onSwitchProfile }: AdminFanafSidebarProps) {
  ...
  const tresorerieSubItems = [
    { id: 'finance' as NavItem, label: 'Vue d\'ensemble' },
    { id: 'finance-paiements' as NavItem, label: 'Liste des paiements' },
  ];
    ...
    <aside className="w-64 bg-white border-r border-gray-200 flex flex-col">
      <div className="p-6">
        <div className="space-y-3">
          <Logo type="fanaf" className="h-10 w-auto object-contain" alt="FANAF" />
          <div>
            <h1 className="text-gray-900">FANAF 2026</h1>
            <p className="text-xs text-gray-500">Administration FANAF</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 space-y-3">
        <div className="p-3 bg-gray-50 rounded-lg">
          ...
          <p className="text-sm text-gray-900">Admin FANAF</p>
        </div>
        {onSwitchProfile && (
          <Button 
